...
<span class="success-banner">DONE! (11/03/25)</span>
...
### Learn to Code in Python, Ch. 08: *Loops*  

These are my personal notes on **loops and iterative control flow** from Boot.dev. This chapter explores how to use `for` and `while` loops, including loop control statements like `break` and `continue`. It emphasizes managing **whitespace** for proper syntax, using `range` for iteration, and applying loops to automate repetitive tasks. Mastering loops is critical for building efficient, scalable programs in domains such as **game development**, **data processing**, and **automation scripts**.  

### Learning Outcomes  

By the end of this chapter, you should be able to:  

- Construct `for` and `while` loops to perform repeated actions in code  
- Utilize `break` and `continue` to control loop execution flow accurately  
- Debug indentation and syntax errors related to loop constructs  
- Explain the purpose of `range` and how it controls loop iteration counts  
- Implement loops effectively in practical workflows for automation or data iteration  
...
These examples illustrate concrete, practical ways loops function in programming tasks such as game development, automation, data processing, UI feedback, and system monitoring, highlighting why mastering loop constructs is foundational for efficient, real-world software engineering.

| #️⃣  | Example                                                   | Details                                                                                          | Practical Applications                                                                                   |
| --- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| 1   | `for i in range(0, 100): print(i)`                        | Prints numbers 0 through 99 using a `for` loop with `range` to avoid repetitive code.             | Automating repetitive tasks like batch processing, data entry, or iterating over user IDs in systems.    |
| 2   | `for i in range(5, end): print(i)`                        | Prints numbers starting at 5 up to `end - 1` by changing the start parameter in `range`.         | Generating subsets of data, paginating results, or iterating partial datasets in applications.           |
| 3   | `for i in range(start, end, -1): print(i)`                | Counts down from `start` to `end + 1` using a negative step in `range` for reversed looping.     | Countdown timers, reverse iterations like undo operations, or processing logs/events backward.          |
| 4   | `while current_health < max_health and enemy_distance > 3:` | Uses `while` loop to regenerate health as long as conditions are met.                             | Game character state updates, sensor monitoring until thresholds, or retry logic with limits.           |
| 5   | `for quest_number in range(start, end, step):` with `continue` | Uses a counter and `continue` to award enchantments only every nth iteration within loop.        | Applying conditional rewards, periodic notifications, or throttled batch processing in applications.    |
| 6   | `for enchantment_strength in range(min_enchantment, max_enchantment + 1): break` | Uses `break` to exit loop after condition met to avoid unnecessary checks.                        | Early stopping in searches, validation checks, or event detection to optimize performance.              |
| 7   | `for i in range(10, 0, -1): print(i)` with conditionals    | Counts down with customized output, printing ellipses and final message at loop end.              | Countdown displays, timed notifications, or game/match start signals.                                   |
| 8   | `while num_potions > 0 and mana < max_mana:`             | Uses `while` loop to consume items until resource depleted or max reached.                        | Resource consumption simulation, inventory management, or retry loops with limited resources.           |
...
- [📒 COURSE NOTES: Learn to Code in Python](#-course-notes-learn-to-code-in-python)
  - [📂 Chapter 08: **Loops**](#-chapter-08-loops)
    ...
    - [Learn to Code in Python, Ch. 08: *Loops*](#learn-to-code-in-python-ch-08-loops)
...
When a character in Fantasy Quest is attacked, the strength of the attack is compared to the strength of the character's defensive enchantments. If the character has any enchantment that is _at least as strong_ as the attack, then the attack is blocked and the character takes no damage.

**Fix the `check_defense` function.**

It checks each defensive enchantment against the attack. If an enchantment is strong enough, it prints that the attack is blocked. In that case, **the loop should stop** instead of checking further… _make sure that it does_!
...
Sure! Here are concise, clear definitions in the requested format:

* **Triangular numbers**: Numbers that can form an equilateral triangle when arranged as dots; each equals the sum of the first *n* natural numbers ((1 + 2 + … + n)).
* **Sum of the first n natural numbers**: The total of all consecutive integers from 1 up to *n*, given by the formula 
  
$$
S = \frac{n(n + 1)}{2}
$$

* **Natural numbers**: The positive whole numbers starting from 1 (i.e., 1, 2, 3, 4, …).
* **XP (Experience Points)**: A numeric value that represents progress or achievement, commonly used in games or apps to measure a player’s growth.
...
Each character starts with `0` XP at level `1`. To reach the next level, they need to _accumulate additional XP_ equal to their current level times 5.
...
> 💡**TIP**: *Learn more about **triangular numbers** and **sum of first n numbers** [here](articles/01.08.01__triangular-numbers-sum-first-n.md).*
...
It consumes potions one at a time _until either_:
