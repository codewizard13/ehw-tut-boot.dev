...
<span class="success-banner">DONE! (06/04/2026)</span>
...
These are my personal notes on **Python error handling, exception types, and defensive code design** from Boot.dev. This chapter teaches how to recognize the difference between **syntax errors** and **runtime exceptions**, how to use **try/except blocks** correctly, and why raising your own exceptions matters for building reliable code. It also explains why catching the right exception type is essential to prevent silent failures and to keep programs running smoothly in real-world contexts like **medical software**, **database management**, and **robotics control systems**.   

### Learning Outcomes  

By the end of this chapter, I should be able to:  

- Describe the difference between syntax errors and exceptions in Python.  
- Use `try`, `except`, and `raise` to handle errors safely across different programs.  
- Identify and prevent common failure cases such as invalid input, missing data, or bad indexes.  
- Explain why specific exception handlers should come before general ones.  
- Write functions that raise meaningful exceptions and let callers decide how to respond.  
...
| #️⃣   | Example | Details |
| --- | ------- | ------- |
| 1   | Catch division by zero | Use `try/except ZeroDivisionError` to avoid crashes in calculations. |
| 2   | Handle missing list items | Use `except IndexError` when accessing list elements safely. |
| 3   | Raise custom exceptions | Signal invalid input with `raise Exception("invalid input")` and let the caller handle it. |
| 4   | Avoid catching your own exceptions | Let the caller handle errors rather than swallowing them inside the function. |
