...
These are my personal notes on **Comparison Operators, If-Else Logic, and Boolean Expressions** from Boot.dev. This chapter teaches how to make decisions in Python using comparisons, conditions, and boolean logic. It covers important **comparison operators**, control flow structures like `if`, `elif`, and `else`, and the use of **Boolean expressions** for effective decision-making. Understanding these concepts is essential for writing reliable, data-driven programs that perform complex evaluations and control execution paths in real-world applications.  

### Learning Outcomes  

By the end of this chapter, you should be able to:  

- Implement decision logic using `if`, `elif`, and `else` statements to control program flow  
- Apply **comparison** and **logical operators** in practical contexts like scoring systems or eligibility checks  
- Debug logic errors, such as incorrect operator use or missing colons  
- Explain how **boolean logic** and short-circuit evaluation work in Python  
- Execute standard workflows for evaluating multiple conditions and improving code readability  
...
</section><!-- END #sec-tags --> 

### Applied Examples

| #️⃣ | Example | Details |
| --- | -------- | -------- |
| 1 | `if player_health <= 0:` | Used to decide when a player is dead or alive in a game. |
| 2 | `return (attack_roll != 1 and attack_roll >= armor_class) or attack_roll == 20` | Combines comparison and boolean operators for game logic. |
| 3 | `should_serve = customer_age >= 21 and not on_break and 5 <= time <= 10` | Real-world example of logic inversion for readability. |
...
```py
def check_mount_rental(time_used, time_purchased):
    rental_expired = time_used >= time_purchased
    if rental_expired:
...
*   If a player's power level is greater than the enemy's defense, that player has an _"advantage"_
*   If the player's power and enemy's defense are equal, they are _"evenly matched"_
*   Otherwise, that player has a _"disadvantage"_.
